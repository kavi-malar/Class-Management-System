...
export class LoginComponent implements OnInit {
  ...
  ngOnInit(): void {
    // Show session-expired or device-mismatch message passed via router state
    const nav = this.router.getCurrentNavigation();
    const state = nav?.extras?.state as { alertMsg?: string } | undefined;
    if (state?.alertMsg) {
      this.errorMsg = state.alertMsg;
    }
    // Also check history state (when navigated programmatically)
    const histState = history.state as { alertMsg?: string };
    if (!this.errorMsg && histState?.alertMsg) {
      this.errorMsg = histState.alertMsg;
    }
  }
