...
/**
 * Feature 1: Notify a teacher that their claimed extra class was cancelled
 * because the original teacher restored their period.
 * Uses the Notification model (in-app) since we don't have the teacher's phone.
 */
exports.notifyTeacherExtraClassCancelled = async (freeSlot, restoredChange) => {
  try {
    const Notification = require('../models/Notification');
    const User         = require('../models/User');

    // Populate teacher if needed
    const teacher = await User.findById(freeSlot.teacher?._id || freeSlot.teacher);
    if (!teacher) return;

    const dayNames = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'];
    const changeDate = new Date(restoredChange.changeDate);
    const dayName    = dayNames[changeDate.getDay()];
    const dateStr    = changeDate.toLocaleDateString('en-IN', { day: '2-digit', month: 'short', year: 'numeric' });

    const message = [
      `⚠️ Extra Class Cancelled`,
      `The original teacher has restored their class for ${dayName} Period ${restoredChange.periodNumber} (${dateStr}).`,
      `Your extra class for this slot has been automatically cancelled.`,
      `Subject: ${freeSlot.subject?.name || 'N/A'}`,
      `- Class Management System`
    ].join('\n');

    // Save as in-app notification for the teacher
    await Notification.create({
      recipient:      teacher._id,
      recipientPhone: teacher.phone || '',
      message,
      type:           'timetable_change',
      timetableChange: restoredChange._id,
      status:         'sent',
    });

    // Also attempt SMS if teacher has a phone number
    if (teacher.phone) {
      await sendSMS(teacher.phone, message).catch(err =>
        console.error(`SMS to teacher ${teacher.phone} failed:`, err.message)
      );
    }

    console.log(`✅ Teacher ${teacher.name} notified: extra class cancelled due to restoration`);
  } catch (err) {
    console.error('❌ notifyTeacherExtraClassCancelled error:', err);
  }
};
